...
// The menu's call functions which are each designed to address a specificaton of the assignment
// I've included a main menu for all the CRUD actions requested, but with 2 Rs  - one for 'regular' read finds, and the other for aggregations
// The create menu includes options for the adding of two new movies.  The update menu manipulate these records, so that part of the code will
...
//
//  NB  - Please note:
// By the time I got around to setting up the Mongo database, I was steered to their cloud based 'Atlas' product
...
// for the code to function, you will need to add the IP that you are using to my Mongo Atlas acccount
// Please log in with my username of 20109017@mail.wit.ie and the password *RosanneBirney*  (The * are part of the password)
// you should see a connection option, which will prompt you to allow the IP that you are using to be added to the account
//
// In terms of references:  in addition to course materials, I used the 'Bro code' intro to Mongo and
// I used Chat GPT v4o for help with the overarching js menu structure (relatively simple), for general syntax and as a reference on functions
// I hope that my submission meets the requirements. 
...
// Menus
const mainMenu = `
Welcome to my CA2 Movie Assigment submission
